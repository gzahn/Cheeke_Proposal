...
path <- "./Data/LetendreITS2" # CHANGE to the directory containing your adaptor-free demultiplexed fastq files when using your own data
...
fns <- sort(list.files(file.path(path), full.names = TRUE, pattern = "ITS_out.fastq.gz")) # make pattern match your FWD reads
...
p1 <- plotQualityProfile(fns[1:4]) + ggtitle("Example forward reads")
...
out <- filterAndTrim(fns, filts_f, # input and output file names as denoted above
                     ...
                     truncLen = c(150), # refers to the lengths at which to truncate Fwd and Rev reads, respectively 
...
p3 <- plotQualityProfile(fns[1:2]) + ggtitle("Unfiltered")
p4 <- plotQualityProfile(filts_f[1:2])+ ggtitle("Filtered") + coord_cartesian(xlim = c(0,300))
p3 / p4
