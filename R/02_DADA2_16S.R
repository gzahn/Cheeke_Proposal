...
# fns <- sort(list.files(file.path(path,"filtered"), full.names = TRUE, pattern = "FWD_filt.fastq.gz")) # make pattern match your FWD reads
# rns <- sort(list.files(file.path(path,"filtered"), full.names = TRUE, pattern = "REV_filt.fastq.gz")) # make pattern match your REV reads
